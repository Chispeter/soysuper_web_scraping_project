...
def get_json(pathname: str = "", actual_list: list = [], actual_dictionary: dict = {}) -> list:
...
def extract_categories(actual_list: list, soup_category: any, index: int):
    ...
    try:
        ...
        # Activar para hacer scraping de todas las categorías posteriores
        get_json(pathname=new_category_dict["nombre_de_ruta"], actual_list=new_category_dict["subcategorías"], actual_dictionary=new_category_dict)
...
def extract_products(actual_dictionary: dict, main_soup: any, pathname: str = ""):
    try:
        ...
        extract_products_from_page(products_soup_array=products_soup_array, actual_dictionary=actual_dictionary, pathname=pathname)
        ...
        if total_number_of_pages != "1":
            for number_of_page in range(2, int(total_number_of_pages)+1):
                ...
                new_soup = get_main_soup(pathname=pathname+"?page="+str(number_of_page))
